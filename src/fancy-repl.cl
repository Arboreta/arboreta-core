...
(defun handle-repl-mouse-events ()
   (iter (for x = (+ (get-internal-real-time) 17))
      (when (or (> (abs *scroll-velocity*) 1) (> (abs *scroll-acceleration*) 1))
       (setf *scroll-acceleration* (alexandria:clamp (* *scroll-acceleration* *scroll-friction*) -15 15))
...
(defun repl-test ()
   ...
   (sb-thread:make-thread 'handle-repl-mouse-events :name "mouseevents-thread")
